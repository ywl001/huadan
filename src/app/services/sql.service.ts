...
export class SqlService {
  ...
  getLbsLocation(lac, ci,mnc = 0) {
    ...
    // console.log(sql);
    return this.http.post(CommonData.sqlUrl, {'sql':sql,'action':'select'})
  }

  selectBySql(sql){
    // console.log('get location:',sql);
    return this.http.post<any>(CommonData.sqlUrl, {'sql':sql,'action':'select'});
  }

  selectContactInfo(){
    return this.http.post<any>(CommonData.sqlUrl,{'sql':"select * from directory",'action':'select'});
  }
  ...
  insertOrUpdateContects(number,name){
    ...
    console.log(sql);
    return this.http.post<any>(CommonData.sqlUrl,{'sql':sql,'action':'edit'});
  }
